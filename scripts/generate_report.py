...
METHODS = {
    ...
    "annealed_pnp": {
        ...
        "description": (
            "Annealed Gaussian perturbation, non-local denoising, blur consistency, "
    ...
    "extreme_channel": {
        ...
        "description": (
            "Dark/bright local-extrema guidance with blur consistency and an "
            "artifact-safety acceptance guard."
        ),
    },
}
METHOD_ORDER = ("baseline", "annealed_pnp", "extreme_channel")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Developer smoke-test limit; Docker validation never uses it.",
    )
    return parser
...
def load_profiles() -> dict[str, dict[str, float | int]]:
    if not PROFILE_FILE.is_file():
        raise RuntimeError(f"missing benchmark profile: {PROFILE_FILE}")
    data = json.loads(PROFILE_FILE.read_text(encoding="utf-8"))
    if not isinstance(data, dict):
        raise RuntimeError("benchmark_profiles.json must contain an object")
    return data
...
def psnr(candidate: np.ndarray, reference: np.ndarray) -> float:
    if candidate.shape != reference.shape:
        raise ValueError("PSNR requires identical shapes")
...
def ssim(candidate: np.ndarray, reference: np.ndarray) -> float:
    ...
    if a.shape != b.shape:
        raise ValueError("SSIM requires identical shapes")
    if a.ndim == 2:
        a, b = a[..., None], b[..., None]
...
def read_kernel(path: Path | None) -> np.ndarray | None:
    ...
    if image is None:
        return None
    kernel = image.astype(np.float32) / 255.0
...
def diagnostics(
    ...
    *,
    ...
    if legacy_reference is not None:
        row["psnr_vs_legacy_db"] = psnr(output, legacy_reference)
        row["ssim_vs_legacy"] = ssim(output, legacy_reference)
    return row
...
def copy_source(source: Path, case_dir: Path) -> str:
    name = f"input{source.suffix.lower()}"
...
def card(title: str, image_path: str, subtitle: str, badge: str) -> str:
    return (
        ...
        f'<img loading="lazy" src="{html.escape(image_path)}" '
        f'alt="{html.escape(title)}"></article>'
    )
...
def method_table(
    ...
    base_rmse = float(baseline["reblur_rmse"])
    base_sharp = float(baseline["sharpness"])
    base_noise = float(baseline["noise_mad"])
    body: list[str] = []
...
def aggregate_metrics(
    ...
    runtime_sets: dict[str, list[float]],
) -> dict[str, dict[str, float | int | None]]:
    aggregate: dict[str, dict[str, float | int | None]] = {}
    for method in METHOD_ORDER:
        rows = metric_sets[method]
        psnrs = [
            float(row["psnr_vs_legacy_db"])
            for row in rows
            if row["psnr_vs_legacy_db"] is not None
        ]
        ssims = [
            float(row["ssim_vs_legacy"])
            for row in rows
            if row["ssim_vs_legacy"] is not None
        ]
        aggregate[method] = {
...
def build_html(
    ...
    agg_rows = "".join(
        ...
        f"<td>{fmt(aggregate[key]['reblur_rmse_mean'], 5)}</td>"
        f"<td>{pct(float(aggregate[key]['reblur_rmse_improvement_vs_baseline_pct']))}</td>"
        f"<td>{fmt(aggregate[key]['sharpness_mean'], 4)}</td>"
        f"<td>{fmt(aggregate[key]['noise_mad_mean'], 5)}</td>"
    ...
    for case in per_image:
        ...
        cards = [
            card(
                ...
                f"Native {shape_text(case['source_shape'])} · exact source bytes copied",
                "Input",
            )
        ]
        ...
        if case["legacy_kernel_shape"] is not None:
            kernel_note = "Previous saved kernel · evaluation only"
            kernel_metric = case["legacy_kernel_metrics"]
            if kernel_metric is not None:
                kernel_note += f" · corr {kernel_metric['correlation']:.4f}"
            cards.append(card("Legacy kernel", f"{rel}/legacy_kernel.png", kernel_note, "Legacy"))
        ...
        cases.append(
            ...
            f'<span class="summary-note">{html.escape(shape_text(case["source_shape"]))} · '
            f"{html.escape(profile_text)}</span></summary>"
            f'<div class="cards">{"".join(cards)}</div>'
            '<div class="table-wrap"><table><thead><tr>'
            '<th>Method</th><th>Stage runtime</th><th>Reblur RMSE ↓</th><th>RMSE gain</th>'
            '<th>Sharpness*</th><th>Sharp Δ</th><th>Noise MAD*</th><th>Noise Δ</th>'
            '<th>PSNR vs legacy*</th><th>SSIM vs legacy*</th></tr></thead>'
            f'<tbody>{method_table(case["metrics"], case["runtimes_seconds"])}</tbody>'
            "</table></div></details>"
        )

    css = """
:root{--bg:#f4f7fb;--ink:#132033;--muted:#64748b;--line:#dfe6ef;--blue:#315efb;
--green:#0b8f68;--shadow:0 12px 34px rgba(25,42,70,.08)}*{box-sizing:border-box}
body{margin:0;background:var(--bg);color:var(--ink);font:14px/1.55 Inter,system-ui,sans-serif}
main{max-width:1540px;margin:auto;padding:36px 22px 64px}.hero{background:linear-gradient(135deg,#0d172c,
#24407f 58%,#49358c);color:#fff;border-radius:25px;padding:34px 38px;box-shadow:var(--shadow)}
.eyebrow{font-size:11px;font-weight:800;letter-spacing:.15em;text-transform:uppercase;color:#bdd0ff}
h1{font-size:34px;line-height:1.13;margin:7px 0 10px}.hero>p{max-width:1080px;color:#dce7ff}
.kpis{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:12px;margin-top:22px}
.kpi{padding:13px 15px;background:#ffffff18;border:1px solid #ffffff26;border-radius:14px}
.kpi b{display:block;font-size:22px}.kpi span{font-size:11px;color:#d1dcf8}section{margin-top:30px}
h2{font-size:23px;margin:0 0 6px}.lead{color:var(--muted)}.notice{background:#eef3ff;border-left:4px
solid var(--blue);padding:14px 16px;border-radius:10px;margin:14px 0;color:#30466f}.notice.good{background:#eaf8f3;
border-left-color:var(--green);color:#245c4b}.methods{display:grid;grid-template-columns:repeat(3,1fr);gap:14px}
.method,.card,.case,.table-wrap,.meta div{background:#fff;border:1px solid var(--line);box-shadow:var(--shadow)}
.method{border-radius:17px;padding:17px}.method b{font-size:16px}.method p,.card p{color:var(--muted)}
.table-wrap{overflow:auto;border-radius:17px;margin-top:14px}table{border-collapse:collapse;width:100%;min-width:1120px}
th,td{padding:11px 13px;border-bottom:1px solid var(--line);text-align:right;vertical-align:top}
th:first-child,td:first-child{text-align:left}th{background:#f8fafc;color:var(--muted);font-size:10px;text-transform:uppercase}
td small{display:block;color:var(--muted);font-weight:400;max-width:430px}.case{margin-top:13px;border-radius:18px;
overflow:hidden}summary{cursor:pointer;padding:15px 17px;display:flex;justify-content:space-between;background:#fbfcfe}
summary b{display:inline-grid;place-items:center;width:29px;height:29px;margin-right:9px;border-radius:9px;background:#eaf0ff;
color:#294fd2}.summary-note{font-size:12px;color:var(--muted)}.cards{display:grid;grid-template-columns:repeat(auto-fit,
minmax(210px,1fr));gap:12px;padding:14px}.card{border-radius:14px;padding:10px}.card-head{display:flex;justify-content:
space-between;gap:7px}.card h4{font-size:13px;margin:0}.card p{font-size:10px;margin:2px 0 8px}.card img{display:block;
width:100%;height:auto;border-radius:9px;background:#eef2f7}.badge{font-size:9px;font-weight:800;text-transform:uppercase;
padding:3px 6px;border-radius:999px;background:#eaf0ff;color:#294fd2;white-space:nowrap}.meta{display:grid;
grid-template-columns:repeat(4,1fr);gap:12px}.meta div{border-radius:14px;padding:13px 15px}.meta b{display:block;font-size:12px;
color:var(--muted)}.meta span{font-family:ui-monospace,monospace;font-size:12px}code{background:#eaf0f5;border-radius:5px;
padding:2px 5px}.foot{font-size:12px;color:var(--muted);margin-top:24px}@media(max-width:900px){.kpis,.meta{grid-template-columns:
1fr 1fr}.methods{grid-template-columns:1fr}}@media(max-width:520px){.kpis,.meta{grid-template-columns:1fr}}
"""
    legacy = report["legacy_comparison"]
    environment = report["environment"]
    return f'''<!doctype html><html lang="en"><head><meta charset="utf-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>Adaptive Blind Deblur · Native-Resolution Benchmark</title><style>{css}</style></head><body><main>
<header class="hero"><div class="eyebrow">Full-quality native-resolution Docker benchmark</div>
<h1>Adaptive blind deblurring · three-method comparison</h1>
<p>Every source image is processed at native dimensions using an explicit quality profile. Previous saved
outputs are evaluation-only and are never supplied to the algorithms.</p><div class="kpis">
<div class="kpi"><b>{report['dataset']['image_count']}</b><span>native source images</span></div>
<div class="kpi"><b>{report['dataset']['image_count'] * 3}</b><span>restored outputs</span></div>
<div class="kpi"><b>{legacy['exact_shape_results']}</b><span>legacy outputs compared</span></div>
<div class="kpi"><b>{legacy['comparable_kernels']}</b><span>legacy kernels comparable</span></div>
<div class="kpi"><b>{report['total_runtime_seconds']:.1f}s</b><span>benchmark runtime</span></div></div></header>
<section><h2>Benchmark contract</h2><div class="notice good"><strong>Native resolution:</strong> every method
must return the exact source dimensions.</div><div class="notice"><strong>Independent inference:</strong>
<code>dataset/benchmark_profiles.json</code> supplies configuration only. Legacy pixels and legacy kernel values
are evaluation-only.</div><div class="notice"><strong>Artifact guard:</strong> refinements are reduced or rejected
when a small consistency gain causes excessive high-frequency amplification.</div></section>
<section><h2>Methods</h2><div class="methods">{''.join(
    f'<div class="method"><b>{html.escape(METHODS[key]["name"])}</b><p>{html.escape(METHODS[key]["description"])}</p></div>'
    for key in METHOD_ORDER
)}</div></section>
<section><h2>Aggregate results</h2><p class="lead">Legacy metrics are regression/fidelity measurements, not
clean-image ground truth.</p><div class="table-wrap"><table><thead><tr><th>Method</th><th>E2E runtime</th>
<th>Reblur RMSE ↓</th><th>RMSE gain</th><th>Sharpness*</th><th>Noise MAD*</th><th>PSNR vs legacy*</th>
<th>SSIM vs legacy*</th><th>Reference n</th></tr></thead><tbody>{agg_rows}</tbody></table></div></section>
<section><h2>Reproducibility metadata</h2><div class="meta"><div><b>Git commit</b><span>{html.escape(report['git_commit'])}</span></div>
<div><b>Python</b><span>{environment['python']}</span></div><div><b>OpenCV / NumPy / SciPy</b>
<span>{environment['opencv']} / {environment['numpy']} / {environment['scipy']}</span></div>
<div><b>Generated UTC</b><span>{html.escape(report['generated_utc'])}</span></div></div></section>
<section><h2>All source images</h2>{''.join(cases)}</section><p class="foot">Use visual inspection together
with measurement consistency and artifact diagnostics; no single metric is a perceptual-quality oracle.</p>
</main></body></html>'''
...
def write_summary(
    ...
    lines.extend(
        [
            "",
            "*Legacy metrics measure agreement with previous saved outputs; they are not ground-truth scores.*",
        ]
    )
...
def generate_report(output_dir: Path = RESULTS, *, limit: int | None = None) -> Path:
    ...
    if len(images) != EXPECTED_IMAGES:
        raise RuntimeError(f"dataset/image has {len(images)} images; expected {EXPECTED_IMAGES}")
    profiles = load_profiles()
    source_names = {path.name for path in images}
    if set(profiles) != source_names:
        raise RuntimeError("benchmark profiles must match dataset/image exactly")
    ...
    per_image: list[dict[str, Any]] = []
    metric_sets: dict[str, list[dict[str, float | None]]] = {
        key: [] for key in METHOD_ORDER
    }
    runtime_sets: dict[str, list[float]] = {key: [] for key in METHOD_ORDER}
    legacy_matches = 0
    ...
    for index, source in enumerate(images, 1):
        ...
        print(
            ...
            f"(kernel {config.kernel_size}×{config.kernel_size})",
            flush=True,
        )
        ...
        baseline, kernel, interim = deblur_image(observed, config)
        ...
        for method, output in outputs.items():
            if output.shape != observed.shape:
                raise RuntimeError(f"{method} changed resolution for {source.name}")
        ...
        legacy_k_metrics = kernel_metrics(kernel, legacy_kernel)
        if legacy_k_metrics is not None:
    ...
    aggregate = aggregate_metrics(metric_sets, runtime_sets)
    report: dict[str, Any] = {
        "schema_version": 2,
        ...
        "dataset": {
            ...
            "profile_note": (
                "Profiles are configuration only; legacy pixels and kernel values are not inference inputs."
            ),
        },
        "legacy_comparison": {
    ...
    report_path.write_text(build_html(report, per_image, aggregate), encoding="utf-8")
    write_summary(
        output_dir,
        len(images),
        total_runtime,
        legacy_matches,
        legacy_kernel_matches,
        aggregate,
    )
